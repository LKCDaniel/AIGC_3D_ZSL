...
def init_render_with_ha(r_p, phi_1, phi_2, img_size=512.0):
    x, y, z = calculate_target_coordinates(r_p, phi_1, phi_2)
...
def render_object_style(rasterizer, renderer, mesh, img_style):
    ...
    if img_style == 'normal':
        ...
        normals_rgb = normals.detach().cpu().numpy()

        if normals_rgb.shape[0] == 3:
